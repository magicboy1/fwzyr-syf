...
const OPTION_SHAPES = ["rounded-lg", "rounded-full", "rounded-lg rotate-45", "rounded-lg"] as const;
...
export default function DisplayScreen() {
  ...
  useEffect(() => {
    ...
    socket.on("game:end", (data) => {
      ...
      setPhase("END");
      setTimeout(() => {
        confetti({ particleCount: 200, spread: 120, origin: { y: 0.6 }, colors: ["#c9a84c", "#f0d78c", "#1e3a5f", "#fff"] });
      }, 500);
      setTimeout(() => {
        confetti({ particleCount: 100, spread: 80, origin: { x: 0.2, y: 0.7 }, colors: ["#c9a84c", "#f0d78c"] });
      }, 1200);
      setTimeout(() => {
        confetti({ particleCount: 100, spread: 80, origin: { x: 0.8, y: 0.7 }, colors: ["#c9a84c", "#f0d78c"] });
      }, 1800);
    });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden relative" data-testid="display-screen">
          ...
          >
            <div className="text-center">
              <motion.div
                ...
                className="text-7xl font-bold bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500 bg-clip-text text-transparent"
              >
                x2
              </motion.div>
              <p className="text-3xl text-amber-200 mt-4 font-semibold">Double Points Question!</p>
              <p className="text-2xl text-amber-200/70 mt-2">!سؤال النقاط المضاعفة</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {streakAlert && (
          ...
          >
            <p className="text-2xl font-bold text-white text-center">
              {streakAlert.playerName} is on fire! {streakAlert.streak} streak
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
function LobbyScreen({ pin, joinUrl, playerCount, players }: { pin: string; joinUrl: string; playerCount: number; players: { id: string; name: string }[] }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex flex-col items-center justify-center p-8" data-testid="lobby-screen">
      <div className="mb-8">
        <img src={logoUrl} alt="Annual Suhoor" className="h-20 mx-auto object-contain opacity-90" data-testid="img-logo" />
      </div>

      <motion.h1 initial={{ y: -30 }} animate={{ y: 0 }} className="text-5xl font-bold bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500 bg-clip-text text-transparent mb-2">
        فوازير سيف
      </motion.h1>
      <p className="text-xl text-muted-foreground mb-12">Fawazeer Seif</p>

      <div className="flex flex-col md:flex-row gap-12 items-center">
        <div className="bg-white p-6 rounded-2xl shadow-2xl" data-testid="qr-code">
          <QRCodeSVG value={joinUrl} size={220} level="H" bgColor="#ffffff" fgColor="#0f1d36" />
        </div>

        <div className="text-center">
          <p className="text-lg text-muted-foreground mb-3">Game PIN</p>
          <div className="flex gap-2 justify-center mb-8" data-testid="text-pin">
            {pin.split("").map((d, i) => (
              <motion.span key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.1 }} className="w-16 h-20 flex items-center justify-center text-4xl font-bold bg-gradient-to-b from-card to-muted rounded-xl border border-border/50 text-primary">
                {d}
              </motion.span>
            ))}
          </div>

          <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2, repeat: Infinity }} className="bg-card/80 backdrop-blur rounded-2xl px-10 py-6 border border-primary/20">
            <p className="text-6xl font-bold text-primary" data-testid="text-player-count">{playerCount}</p>
            <p className="text-lg text-muted-foreground mt-1">Players joined</p>
          </motion.div>
        </div>
      </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex flex-col p-8 lg:p-12" data-testid="question-screen">
      <div className="flex items-center justify-between mb-6 gap-4">
        ...
        {question.isDoublePoints && (
          <span className="px-4 py-1 bg-gradient-to-r from-amber-500/20 to-yellow-500/20 border border-amber-500/40 rounded-full text-amber-300 text-sm font-semibold">
            x2 Double Points
          </span>
        )}
        <div className="flex items-center gap-3">
          {paused && <span className="text-amber-400 font-semibold">PAUSED</span>}
          <span className="text-3xl font-bold text-primary tabular-nums" data-testid="text-timer">
            {Math.ceil(timeLeft)}
          </span>
        </div>
      </div>
      ...
      <div className="w-full h-3 bg-muted rounded-full overflow-hidden mb-10">
        <motion.div
          ...
          style={{
            width: `${timerPercent}%`,
            background: timerPercent > 30 ? "linear-gradient(90deg, hsl(43 72% 52%), hsl(43 72% 65%))" : timerPercent > 10 ? "linear-gradient(90deg, hsl(30 80% 50%), hsl(43 72% 52%))" : "linear-gradient(90deg, hsl(0 70% 50%), hsl(30 80% 50%))",
          }}
...
function RevealScreen({ reveal, question }: { reveal: QuestionReveal; question: QuestionForBigScreen | null }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex flex-col p-8 lg:p-12" data-testid="reveal-screen">
      <div className="mb-6">
        <p className="text-muted-foreground text-lg mb-1">Question {reveal.questionIndex + 1}</p>
        {question && <h2 className="text-2xl font-bold" dir="auto">{question.text}</h2>}
      </div>

      {reveal.isDoublePoints && (
        <div className="text-center mb-4">
          <span className="px-4 py-1 bg-amber-500/20 border border-amber-500/40 rounded-full text-amber-300 text-sm font-semibold">x2 Double Points</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
        {reveal.topFastest.length > 0 && (
          <div className="bg-card/50 rounded-2xl p-6 border border-border/30">
            <h3 className="text-lg font-semibold text-primary mb-4">Fastest Correct</h3>
            {reveal.topFastest.map((p, i) => (
              <motion.div key={i} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.2 }} className="flex items-center gap-4 mb-3">
                <span className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/30 to-primary/10 flex items-center justify-center font-bold text-primary text-sm">{i + 1}</span>
                <span className="font-medium flex-1" dir="auto">{p.name}</span>
                <span className="text-muted-foreground">{(p.timeMs / 1000).toFixed(1)}s</span>
              </motion.div>
            ))}
          </div>
        )}
        ...
        <div className="bg-card/50 rounded-2xl p-6 border border-border/30">
          <h3 className="text-lg font-semibold text-primary mb-4">Top 5</h3>
          {reveal.leaderboard.slice(0, 5).map((entry, i) => (
            <motion.div key={entry.playerId} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.15 }} className="flex items-center gap-4 mb-3">
              <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${i === 0 ? "bg-gradient-to-br from-amber-400 to-yellow-600 text-white" : i === 1 ? "bg-gradient-to-br from-gray-300 to-gray-500 text-white" : i === 2 ? "bg-gradient-to-br from-orange-400 to-orange-600 text-white" : "bg-muted text-muted-foreground"}`}>{entry.rank}</span>
              ...
              <span className="font-bold text-primary tabular-nums">{entry.score.toLocaleString()}</span>
            </motion.div>
          ))}
        </div>
      </div>
...
function LeaderboardScreen({ leaderboard }: { leaderboard: LeaderboardEntry[] }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex flex-col items-center justify-center p-8 lg:p-12" data-testid="leaderboard-screen">
      <h2 className="text-4xl font-bold bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500 bg-clip-text text-transparent mb-12">Leaderboard</h2>
      <div className="w-full max-w-2xl">
        {leaderboard.slice(0, 10).map((entry, i) => (
          <motion.div
            ...
            transition={{ delay: i * 0.1 }}
            className={`flex items-center gap-6 mb-4 p-4 rounded-xl ${i < 3 ? "bg-gradient-to-r from-primary/10 to-transparent border border-primary/20" : "bg-card/50 border border-border/20"}`}
            data-testid={`leaderboard-entry-${i}`}
          >
            <span className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${i === 0 ? "bg-gradient-to-br from-amber-400 to-yellow-600 text-white shadow-lg shadow-amber-500/30" : i === 1 ? "bg-gradient-to-br from-gray-300 to-gray-400 text-gray-800" : i === 2 ? "bg-gradient-to-br from-orange-400 to-orange-600 text-white" : "bg-muted text-muted-foreground"}`}>
              {entry.rank}
            </span>
            ...
            <span className="text-2xl font-bold text-primary tabular-nums">{entry.score.toLocaleString()}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
...
function EndScreen({ stats }: { stats: FinalStats }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen flex flex-col items-center p-8 lg:p-12 overflow-y-auto" data-testid="end-screen">
      <img src={logoUrl} alt="Logo" className="h-12 mb-6 opacity-80" />
      <h2 className="text-4xl font-bold bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500 bg-clip-text text-transparent mb-4">Game Over!</h2>
        ...
        <div className="flex items-end justify-center gap-6 mb-12 mt-8">
          {stats.podium.length > 1 && (
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="flex flex-col items-center">
              ...
              <p className="text-primary font-bold">{stats.podium[1].score.toLocaleString()}</p>
          ...
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="flex flex-col items-center -mb-4">
            <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2, repeat: Infinity }} className="w-28 h-28 rounded-full bg-gradient-to-br from-amber-400 via-yellow-500 to-amber-600 flex items-center justify-center text-4xl font-bold text-white mb-3 shadow-2xl shadow-amber-500/40">
              1
            </motion.div>
            <p className="font-bold text-2xl mb-1" dir="auto">{stats.podium[0].name}</p>
            <p className="text-primary font-bold text-xl">{stats.podium[0].score.toLocaleString()}</p>
            <div className="w-28 h-32 bg-gradient-to-t from-primary/20 to-primary/5 rounded-t-lg mt-3" />
          </motion.div>
          ...
          {stats.podium.length > 2 && (
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.9 }} className="flex flex-col items-center">
              ...
              <p className="text-primary font-bold">{stats.podium[2].score.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-4xl mb-8">
        {stats.fastestCorrect && (
          <StatCard title="Fastest Answer" value={`${(stats.fastestCorrect.timeMs / 1000).toFixed(1)}s`} subtitle={stats.fastestCorrect.playerName} />
        )}
        {stats.bestStreak && (
          <StatCard title="Best Streak" value={`${stats.bestStreak.streakLength} in a row`} subtitle={stats.bestStreak.playerName} />
        )}
        {stats.hardestQuestion && (
          <StatCard title="Hardest Question" value={`${stats.hardestQuestion.correctPercent}% correct`} subtitle={`Q${stats.hardestQuestion.questionIndex + 1}`} />
        )}
        <StatCard title="Avg Response Time" value={`${(stats.avgResponseTime / 1000).toFixed(1)}s`} subtitle="All players" />
        <StatCard title="Participation" value={`${stats.participationRate}%`} subtitle={`${stats.totalPlayers} players`} />
        <StatCard title="Total Questions" value={`${stats.totalQuestions}`} subtitle="Questions played" />
      </div>
    </motion.div>
  );
}
